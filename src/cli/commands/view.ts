...
/**
 * A review id names a directory under the repository's own review directory; a
 * path is accepted too, and then checked to be inside it. Neither form may
 * escape: a saved review is repository state, not an arbitrary file to serve.
 */
async function resolveReviewDirectory(
  ...
  const reviewsRoot = path.join(repositoryRoot, REVIEWS_DIR);

  let candidate: string;
  if (REVIEW_ID.test(requested) && !requested.includes(path.sep) && !requested.endsWith('.json')) {
    candidate = path.join(reviewsRoot, requested);
  } else {
  ...
  const boundary = await resolveLinks(runtime, path.resolve(reviewsRoot));
  if (normalized !== boundary && !normalized.startsWith(`${boundary}${path.sep}`)) {
    throw new AmbicodeError('review-outside-repository', 'That review is not inside this repository.', {
      field: '--review',
      details: [
        `Saved reviews live under ${REVIEWS_DIR}/ in the repository this command runs in.`,
        'Pass a review id, or a path inside that directory.',
      ],
    });
  }
